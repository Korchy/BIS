...
class BISMeshPanel(Panel):
    ...
    def draw(self, context):
        ...
        if WebAuthVars.logged:
            if context.preferences.addons[__package__].preferences.default_mode_in_3d_view == 'MESHES':
                # meshes
                ...
                layout.template_icon_view(
                    context.window_manager.bis_get_meshes_info_from_storage_vars,
                    'items',
                    show_labels=True
                )
                layout.separator()
                ...
                # materials / geometry noes (simple)
                if WebAuthVars.userProStatus:
